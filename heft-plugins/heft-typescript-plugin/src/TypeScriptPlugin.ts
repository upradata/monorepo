...
import type { ITypeScriptConfigurationJson } from './plugin-options';
...
export default class TypeScriptPlugin implements IHeftTaskPlugin {
    ...
    private async _getStaticAssetCopyOperations(
        ...
        copyOperations.forEach(d => taskSession.logger.terminal.writeVerboseLine(JSON.stringify(d)));
    ...
    private async _getTypeScriptBuilderAsync(
        ...
        if (!partialTsconfigFile) {
            terminal.writeLine(`No tsconfig file found in "${heftConfiguration.buildFolderPath}". Skipping ${PLUGIN_NAME}.`);
        ...
        const typeScriptBuilderConfiguration: ITypeScriptBuilderConfiguration = {
            ...
            useTsconfigAsBase: typeScriptConfigurationJson?.useTsconfigAsBase,
